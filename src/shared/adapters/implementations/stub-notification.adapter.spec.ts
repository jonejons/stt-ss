...
import { EmailOptions, SmsOptions, PushNotificationOptions } from '../notification.adapter';
...
describe('StubNotificationAdapter', () => {
  ...
  describe('template management', () => {
    ...
    it('should throw error for non-existing template', async () => {
      await expect(adapter.getTemplate('non-existing'))
        .rejects.toThrow('Template not found: non-existing');
    });
    ...
    it('should delete a template', async () => {
      ...
      // Verify it's deleted
      await expect(adapter.getTemplate(template.id))
        .rejects.toThrow(`Template not found: ${template.id}`);
    });
  });
});