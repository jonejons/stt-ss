...
describe('QueueService', () => {
  ...
  describe('addEventJob', () => {
    ...
    it('should handle job addition failure', async () => {
      ...
      await expect(service.addEventJob('test-event', jobData))
  ...
  describe('getQueueStats', () => {
    ...
    it('should throw error for unknown queue', async () => {
      await expect(service.getQueueStats('unknown'))
  ...
  describe('getAllQueueStats', () => {
    it('should return statistics for all queues', async () => {
      ...
      [mockEventsQueue, mockNotificationsQueue, mockExportsQueue, mockSystemHealthQueue]
        .forEach(queue => {
          Object.keys(mockStats).forEach(method => {
            queue[`get${method.charAt(0).toUpperCase() + method.slice(1)}`]
