...
export class AttendanceController {
  ...
  async getLiveAttendance(
    ...
    todayRecords.forEach(record => {
      ...
      if (status.lastCheckIn && (!status.lastCheckOut || status.lastCheckIn > status.lastCheckOut)) {
  ...
  async getWeeklyAttendanceReport(
    ...
    return this.attendanceService.getWeeklyAttendanceReport(new Date(startDate), branchId, scope);
  }
