...
describe('EmployeeService', () => {
  ...
  describe('createEmployee', () => {
    ...
    it('should create an employee successfully', async () => {
      ...
      expect(employeeRepository.findByEmployeeCode).toHaveBeenCalledWith('EMP001', mockDataScope);
    ...
    it('should throw BadRequestException for inaccessible branch', async () => {
      ...
      await expect(service.createEmployee(invalidDto, mockDataScope, 'user-123'))
    ...
    it('should throw ConflictException for duplicate employee code', async () => {
      ...
      await expect(service.createEmployee(createDto, mockDataScope, 'user-123'))
  ...
  describe('getEmployeesByBranch', () => {
    it('should return employees for accessible branch', async () => {
      ...
      expect(employeeRepository.findByBranch).toHaveBeenCalledWith('branch-123', mockDataScope);
      expect(result).toEqual(employees);
    });

    it('should throw BadRequestException for inaccessible branch', async () => {
      await expect(service.getEmployeesByBranch('invalid-branch', mockDataScope))
        .rejects.toThrow(BadRequestException);
    });
  });
  ...
  describe('updateEmployee', () => {
    ...
    it('should update an employee successfully', async () => {
      const updatedEmployee = { ...mockEmployee, firstName: 'Jane', email: 'jane.doe@example.com' };
      
      employeeRepository.findById.mockResolvedValue(mockEmployee);
      employeeRepository.update.mockResolvedValue(updatedEmployee);

      const result = await service.updateEmployee('emp-123', updateDto, mockDataScope, 'user-123');

      expect(employeeRepository.findById).toHaveBeenCalledWith('emp-123', mockDataScope);
      expect(employeeRepository.update).toHaveBeenCalledWith('emp-123', updateDto, mockDataScope);
      expect(loggerService.logUserAction).toHaveBeenCalledWith(
        'user-123',
        'EMPLOYEE_UPDATED',
        expect.objectContaining({
    ...
    it('should throw NotFoundException when employee not found', async () => {
      ...
      await expect(service.updateEmployee('nonexistent', updateDto, mockDataScope, 'user-123'))
    ...
    it('should throw ConflictException for duplicate employee code', async () => {
      ...
      await expect(service.updateEmployee('emp-123', updateWithCode, mockDataScope, 'user-123'))
        .rejects.toThrow(ConflictException);
    });
  });
  ...
  describe('deleteEmployee', () => {
    ...
    it('should throw NotFoundException when employee not found', async () => {
      ...
      await expect(service.deleteEmployee('nonexistent', mockDataScope, 'user-123'))
        .rejects.toThrow(NotFoundException);
    });
  });
  ...
  describe('toggleEmployeeStatus', () => {
    it('should activate an employee successfully', async () => {
      ...
      const result = await service.toggleEmployeeStatus('emp-123', true, mockDataScope, 'user-123');

      expect(employeeRepository.findById).toHaveBeenCalledWith('emp-123', mockDataScope);
      expect(employeeRepository.update).toHaveBeenCalledWith('emp-123', { isActive: true }, mockDataScope);
      expect(loggerService.logUserAction).toHaveBeenCalledWith(
        'user-123',
        'EMPLOYEE_ACTIVATED',
        expect.objectContaining({
    ...
    it('should deactivate an employee successfully', async () => {
      ...
      const result = await service.toggleEmployeeStatus('emp-123', false, mockDataScope, 'user-123');
  ...
  describe('getEmployeeCountByBranch', () => {
    it('should return count for accessible branch', async () => {
      ...
      expect(employeeRepository.count).toHaveBeenCalledWith({ branchId: 'branch-123' }, mockDataScope);
      expect(result).toBe(5);
    });

    it('should throw BadRequestException for inaccessible branch', async () => {
      await expect(service.getEmployeeCountByBranch('invalid-branch', mockDataScope))
        .rejects.toThrow(BadRequestException);
    });
  });
});