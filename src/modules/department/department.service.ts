...
export class DepartmentService {
  ...
  async createDepartment(
    ...
    try {
      ...
      if (createDepartmentDto.parentId) {
        ...
        if (!isValidParent) {
          throw new BadRequestException('Invalid parent department or parent department not in the same branch');
        }
      }
      ...
      if (DatabaseUtil.isUniqueConstraintError(error)) {
        ...
        throw new ConflictException(`Department with this ${fields.join(', ')} already exists in this branch`);
      }
      throw error;
    }
  }
  ...
  async updateDepartment(
    ...
    try {
      ...
      if (updateDepartmentDto.parentId) {
        ...
        const wouldCreateCircularRef = await this.departmentRepository.checkCircularReference(
          id,
          updateDepartmentDto.parentId,
        );

        if (wouldCreateCircularRef) {
          throw new BadRequestException('Cannot set parent department: would create circular reference');
        }
      ...
      const updatedDepartment = await this.departmentRepository.update(id, updateDepartmentDto, scope);
  ...
  async deleteDepartment(
    ...
    if (childrenCount > 0) {
      throw new BadRequestException('Cannot delete department with child departments. Please delete or reassign child departments first.');
    }
