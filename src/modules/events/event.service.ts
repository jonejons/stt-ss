...
export class EventService {
  ...
  async processRawEvent(
    ...
    const eventLog = await this.eventRepository.createDeviceEventLog({
      ...
      timestamp: createRawEventDto.timestamp ? new Date(createRawEventDto.timestamp) : new Date(),
  ...
  private generateMockSignature(deviceId: string, eventData: CreateRawEventDto): string {
    ...
    return crypto.createHash('sha256').update(payload + 'mock-secret').digest('hex').substring(0, 32);
  }
